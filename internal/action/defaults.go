...
func DefaultPRActions() map[string]Action {
	return map[string]Action{
		...
		"m": {Key: "m", Label: "Merge (squash)",
			Command: Command{Argv: []string{"gh", "pr", "merge", "{{.Number}}", "--squash"}, Native: "merge-squash"},
		...
		"A": {Key: "A", Label: "Auto-merge (squash)",
			Command:  Command{Argv: []string{"gh", "pr", "merge", "{{.Number}}", "--auto", "--squash"}, Native: "auto-merge-squash"},
			Scope:    "per-selected",
		...
		"o": {Key: "o", Label: "Open in browser",
			Command: Command{Argv: []string{"gh", "pr", "view", "{{.Number}}", "--web"}, Native: "open-web"}, Scope: "per-selected"},
		"W": {Key: "W", Label: "Bulk worktrees",
			Command:  Command{Argv: []string{"wt", "switch", "{{.Branch}}"}},
			ExitsTUI: true, Scope: "per-selected"},
		"u": {Key: "u", Label: "Update branch",
			Command: Command{Argv: []string{"gh", "pr", "update-branch", "{{.Number}}"}, Native: "update-branch"}, Scope: "per-selected", Refresh: true,
			Progress: "Updating branch", Past: "Branch updated", Fail: "Update failed"},
		"M": {Key: "M", Label: "Mark ready",
			Command: Command{Argv: []string{"gh", "pr", "ready", "{{.Number}}"}, Native: "mark-ready"}, Scope: "per-selected", Refresh: true,
...
func DefaultIssueActions() map[string]Action {
	return map[string]Action{
		...
		"W": {Key: "W", Label: "Bulk worktrees",
			...
			ExitsTUI: true, Scope: "per-selected"},
		"o": {Key: "o", Label: "Open in browser",
			Command: Command{Argv: []string{"gh", "issue", "view", "{{.Number}}", "--web"}, Native: "open-web"}, Scope: "per-selected"},
