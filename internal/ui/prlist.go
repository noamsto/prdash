...
func (m Model) cursorCard() (triage.Card, bool) {
	...
	return triage.Compute(ps.prAt(m.cursor), d, m.viewerLogin, ps.stackParentNumber(m.cursor)), true
}
...
// the bar's counterpart to the row's columnLadder. What stays at shedNever is
// every escape hatch: the focused row's own action, the actions palette, the
// filter, the legend, and quit.
const (
	shedNever    = 0
